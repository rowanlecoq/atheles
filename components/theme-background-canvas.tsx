...
// Light-mode palette — same dark atmospheric look as MOBILE_BG but with the
// base lifted ~10–15% lightness and radial opacities boosted ~1.6× for more
// vivid glows. Result: dark mode feel, noticeably brighter, not a reversal.
const LIGHT_BG: Record<ThemeKey, { base: string; radial: RLayer[]; linear?: LStop[] }> = {
  gold: { base: '#2a1800', radial: [
    [0.50, 0.62, 1.45, 0.80, 190, 158, 88,  0.32, 0.88],
    [0.50, 0.48, 0.88, 0.72, 210, 182, 118, 0.50, 0.72],
    [0.50, 0.28, 0.55, 0.48, 228, 200, 138, 0.24, 0.65],
  ]},
  water: { base: '#081e30', radial: [
    [0.50, 1.22, 1.00, 0.40,  20, 100, 200, 0.22, 0.48],
    [0.50, 0.70, 0.65, 0.50,   0, 160, 200, 0.26, 0.55],
    [0.50, 0.50, 0.55, 0.55,  40, 210, 240, 0.28, 0.55],
    [0.70, 0.65, 0.60, 0.70,  20, 100, 200, 0.42, 0.60],
    [0.50, 0.35, 0.70, 0.60,  30, 180, 220, 0.48, 0.60],
  ]},
  tropical: { base: '#081f0f', radial: [
    [0.50, 0.85, 1.20, 0.50,  10, 130,  72, 0.38, 0.60],
    [0.78, 0.72, 0.72, 0.55, 255, 190,  50, 0.42, 0.55],
    [0.50, 0.48, 0.68, 0.68,  12, 155, 100, 0.38, 0.55],
    [0.48, 0.22, 0.68, 0.72, 255, 170,  11, 0.44, 0.58],
    [0.18, 0.65, 0.82, 0.68,   8, 138,  78, 0.50, 0.58],
    [0.72, 0.88, 0.65, 0.52, 255, 162,  18, 0.38, 0.56],
    [0.28, 0.90, 0.62, 0.48,  10, 142,  82, 0.36, 0.58],
  ]},
  midnight: { base: '#100828', radial: [
    [0.12, 0.10, 0.62, 0.55,  55,  18, 205, 0.32, 0.72],
    [0.42, 0.28, 0.70, 0.64,  95,  28, 232, 0.28, 0.75],
    [0.72, 0.50, 0.74, 0.68,  15,  52, 210, 0.36, 0.75],
    [0.82, 0.78, 0.60, 0.58,  80,  20, 218, 0.32, 0.72],
    [0.22, 0.70, 0.62, 0.58,  38,  10, 185, 0.26, 0.70],
    [0.50, 0.42, 0.68, 0.60, 110,  40, 240, 0.22, 0.68],
    [0.06, 0.88, 0.55, 0.50,  58,  14, 200, 0.28, 0.68],
  ]},
  sunset: { base: '#1e0812', radial: [
    [0.50, 1.18, 1.10, 0.40, 255, 140,  20, 0.36, 0.48],
    [0.20, 0.12, 0.55, 0.52, 147,  51, 234, 0.78, 0.60],
    [0.78, 0.28, 0.65, 0.55, 255,  80, 150, 0.80, 0.60],
    [0.15, 0.52, 0.60, 0.55, 219,  39, 119, 0.68, 0.55],
    [0.80, 0.68, 0.55, 0.55, 255, 100, 160, 0.56, 0.55],
    [0.50, 0.55, 1.00, 0.28, 249, 115,  22, 0.42, 0.55],
  ], linear: [
    [147,  51, 234, 0.44, 0.00],
    [147,  51, 234, 0.00, 0.35],
    [249, 115,  22, 0.00, 0.68],
    [255, 140,  20, 0.32, 1.00],
  ]},
};
...
function drawRays(ctx: CanvasRenderingContext2D, w: number, h: number, theme: ThemeKey, time: number) {
  ...
  if (theme === "water") {
    const eCol = "34,211,238";
    const mCol = "6,182,212";
    for (let i = 0; i < 8; i++) {
