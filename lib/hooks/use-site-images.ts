...
/**
 * Reads from server-provided context — identical data on SSR and client.
 * Zero flash, zero re-renders from data loading.
 */
export function useSiteSlideshow(key: string) {
  const ctx = useSiteImagesContext();
  const slot = ctx[key] || defaultSlot(key);
  ...
  // Slideshow timer — only runs client-side when multiple media
  useEffect(() => {
